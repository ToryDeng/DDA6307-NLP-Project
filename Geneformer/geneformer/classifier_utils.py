...
def remove_rare(data, rare_threshold, label, nproc):
    if rare_threshold > 0:
        ...
        nonrare_label_dict = {label: [k for k, v in label_counter if (v / total_cells) > rare_threshold]}
...
def label_classes(classifier, data, gene_class_dict, nproc):
    if classifier == "cell":
        ...
        if len(data) == 0:
            logger.error("No cells remain after filtering for target genes. Check target gene list.")
            raise
    ...
    def classes_to_ids(example):
        if classifier == "cell":
            ...
            example["labels"] = label_gene_classes(example, class_id_dict, gene_class_dict)
        return example
...
def label_gene_classes(example, class_id_dict, gene_class_dict):
    return [class_id_dict.get(gene_class_dict.get(token_id, -100), -100) for token_id in example["input_ids"]]
...
def prep_gene_classifier_split(data, targets, labels, index, subset_name, max_ncells, iteration_num, num_proc):
    # generate cross-validation splits
...
def balance_attr_splits(
    ...
    for i in range(max_trials):
        if not all(count > 1 for count in list(Counter(metadata_df[state_key]).values())):
            logger.error(
        ...
        for state in set(metadata_df[state_key]):
            eval_base += list(
                metadata_df.loc[
                    metadata_df[state_key][metadata_df[state_key].eq(state)].sample(1, random_state=i).index
                ]["split_attr_ids"]
            )
        ...
        pvals = []
        for attr in attr_to_balance:
            train_attr = list(metadata_df[metadata_df["split_attr_ids"].isin(train_ids)][attr])
            eval_attr = list(metadata_df[metadata_df["split_attr_ids"].isin(eval_ids)][attr])
            if attr == state_key:
            ...
            if all(isinstance(item, (int, float)) for item in train_attr + eval_attr):
                ...
                exp = [exp_counts[cat] * sum(obs) / sum(exp_counts.values()) for cat in all_categ]
        ...
        valid_pvals = [pval_i for pval_i in pvals if isinstance(pval_i, (int, float)) and not np.isnan(pval_i)]
        if all(i >= pval_threshold for i in valid_pvals):
            data_dict["train"] = pu.filter_by_dict(data, {attr_to_split: balance_df_i["train_ids"][0]}, nproc)
            data_dict["test"] = pu.filter_by_dict(data, {attr_to_split: balance_df_i["eval_ids"][0]}, nproc)
...
def load_best_model(directory, model_type, num_classes, mode="eval"):
    ...
    file_df = pd.DataFrame({"dir": file_dict.keys(), "eval_macro_f1": file_dict.values()})
    model_superdir = (
        "run-" + file_df.iloc[file_df["eval_macro_f1"].idxmax()]["dir"].split("_objective_")[2].split("_")[0]
    )
