...
class Classifier:
        ...
        **Parameters:**
        ...
        if self.classifier == "gene":
            ...
            self.gene_class_dict = {
                k: set([self.gene_token_dict.get(gene) for gene in v]) for k, v in self.gene_class_dict.items()
            }
            empty_classes = []
    ...
    def validate_options(self):
        ...
        for attr_name, valid_options in self.valid_option_dict.items():
            ...
            if valid_type:
                continue
            logger.error(f"Invalid option for {attr_name}. " f"Valid options for {attr_name}: {valid_options}")
            raise
        ...
        if self.filter_data is not None:
            for key, value in self.filter_data.items():
                if not isinstance(value, list):
                    ...
                    logger.warning(
                        "Values in filter_data dict must be lists. " f"Changing {key} value to list ([{value}])."
                    )
    ...
    def prepare_data(
        ...
        **Parameters**
        ...
        data, id_class_dict = cu.label_classes(self.classifier, data, self.gene_class_dict, self.nproc)

        # save id_class_dict for future reference
        id_class_output_path = (Path(output_directory) / f"{output_prefix}_id_class_dict").with_suffix(".pkl")
        ...
        if split_id_dict is not None:
            ...
            train_data_output_path = (
                (Path(output_directory) / f"{output_prefix}_labeled_train").with_suffix(".dataset").as_posix()
            )
            test_data_output_path = (
                (Path(output_directory) / f"{output_prefix}_labeled_test").with_suffix(".dataset").as_posix()
            )
            ...
            if 1 > test_size > 0:
                if attr_to_split is None:
                    ...
                    train_data_output_path = (Path(output_directory) / f"{output_prefix}_labeled_train").with_suffix(
                        ".dataset"
                    )
                    test_data_output_path = (Path(output_directory) / f"{output_prefix}_labeled_test").with_suffix(
                        ".dataset"
                    )
                    data_dict["train"].save_to_disk(train_data_output_path)
                    data_dict["test"].save_to_disk(test_data_output_path)
                    ...
                    balance_df.to_csv(f"{output_directory}/{output_prefix}_train_test_balance_df.csv")
                    train_data_output_path = (Path(output_directory) / f"{output_prefix}_labeled_train").with_suffix(
                        ".dataset"
                    )
                    test_data_output_path = (Path(output_directory) / f"{output_prefix}_labeled_test").with_suffix(
                ...
                data_output_path = (Path(output_directory) / f"{output_prefix}_labeled").with_suffix(".dataset")
    ...
    def train_all_data(
        ...
        tune_method: Optional[Literal["qlora"]] = None,
    ):
        """
        ...
        **Output**
        ...
        if self.classifier == "gene":
            ...
            labels = pu.flatten_list(
                [[class_id_dict[label]] * len(targets) for label, targets in self.gene_class_dict.items()]
            )
            assert len(targets) == len(labels)
            data = cu.prep_gene_classifier_all_data(data, targets, labels, self.max_ncells, self.nproc)

        trainer = self.train_classifier(model_directory, num_classes, data, None, output_dir, False, tune_method)
    ...
    def validate(
        ...
        **Parameters**
        ...
        if self.classifier == "cell":
            for i in trange(self.num_crossval_splits):
                ...
                if self.num_crossval_splits == 1:
                    ...
                    if split_id_dict is not None:
                        ...
                        balance_df.to_csv(f"{output_dir}/{output_prefix}_train_valid_balance_df.csv")
                    else:
                        data_dict = data.train_test_split(
            ...
            for train_index, eval_index, test_index in tqdm(skf.split(targets, labels, test_ratio)):
                ...
                if n_hyperopt_trials == 0:
                    ...
                    model = cu.load_best_model(ksplit_output_dir, self.model_type, num_classes)
        ...
        if save_eval_output is True:
            eval_metrics_output_path = (Path(output_dir) / f"{output_prefix}_eval_metrics_dict").with_suffix(".pkl")
    ...
    def hyperopt_classifier(
        ...
        **Parameters**
        ...
        model = pu.load_model(self.model_type, num_classes, model_directory, "train", tune_method)
        ...
        trainer.hyperparameter_search(
            ...
            hp_space=lambda _: def_ray_config if self.ray_config is None else self.ray_config,
    ...
    def evaluate_model(
        ...
        **Parameters**
        ...
        if predict is True:
            ...
            pred_dict_output_path = (Path(output_directory) / f"{output_prefix}_pred_dict").with_suffix(".pkl")
    ...
    def evaluate_saved_model(
        ...
        **Parameters**
        ...
        model = pu.load_model(self.model_type, num_classes, model_directory, "eval", tune_method)
        ...
        all_metrics["all_roc_metrics"] = all_roc_metrics
        test_metrics_output_path = (Path(output_directory) / f"{output_prefix}_test_metrics_dict").with_suffix(".pkl")
    ...
    def plot_roc(
        ...
        **Parameters**
        ...
        eu.plot_ROC(roc_metric_dict, model_style_dict, title, output_directory, output_prefix)
    ...
    def plot_predictions(
        ...
        **Parameters**
        ...
        eu.plot_predictions(predictions_df, title, output_directory, output_prefix, kwargs_dict)
