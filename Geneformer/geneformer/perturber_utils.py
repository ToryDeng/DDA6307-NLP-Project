...
def filter_data_by_start_state(filtered_input_data, cell_states_to_model, nproc):
    ...
    if start_state not in state_values:
        logger.error(f"Start state {start_state} is not present " f"in the dataset's {state_key} attribute.")
        raise
...
def _load_model(
    model_type: Literal["Pretrained", "GeneClassifier", "CellClassifier"],
    num_classes: int = None,
    model_directory: Union[str, bytes, os.PathLike] = None,
...
def get_task_type(model_type: Literal["Pretrained", "GeneClassifier", "CellClassifier"]):
    if model_type == "Pretrained":
        raise NotImplementedError("PEFT doesn't support MaskedLM.")
    elif model_type == "GeneClassifier":
...
def get_lora_config(model_type: Literal["Pretrained", "GeneClassifier", "CellClassifier"]):
    return LoraConfig(
        task_type=get_task_type(model_type),
        r=8,
        lora_alpha=8,
        target_modules=["dense", "key", "value", "query"],  # exclude the classifier layer
        lora_dropout=0.05,
...
def load_model(
    model_type: Literal["Pretrained", "GeneClassifier", "CellClassifier"],
    num_classes: int,
    model_directory: Union[str, bytes, os.PathLike],
    mode: Literal["eval", "train"],
    tune_method: Optional[Literal["lora", "qlora"]] = None,
):
    if mode == "eval":  # if eval mode, put the model in eval mode for fwd pass
        if tune_method:
            if tune_method == "lora":
                model = _load_model(model_type, num_classes, model_directory, True, torch_dtype=torch.float32)
            elif tune_method == "qlora":
        ...
        if tune_method == "lora":
            ...
            peft_config = get_lora_config(model_type)
...
def make_perturbation_batch(
    ...
    ## to implement
        ...
        indices_to_perturb = [
            sorted([anchor_index, i]) if i != anchor_index else None for i in range(example_cell["length"][0])
        ]
        indices_to_perturb = [item for item in indices_to_perturb if item is not None]
    else:
        example_input_ids = example_cell["input_ids"][0]
        indices_to_perturb = [
            [example_input_ids.index(token)] if token in example_input_ids else None for token in tokens_to_perturb
        ]
        indices_to_perturb = [item for item in indices_to_perturb if item is not None]
...
def gen_attention_mask(minibatch_encoding, max_len=None):
    ...
    attention_mask = [
        [1] * original_len + [0] * (max_len - original_len) if original_len <= max_len else [1] * max_len
...
def validate_cell_states_to_model(cell_states_to_model):
    if cell_states_to_model is not None:
        ...
        elif set(cell_states_to_model.keys()).issuperset({"state_key", "start_state", "goal_state"}):
            if (
                ...
                logger.error("Please specify 'state_key', 'start_state', and 'goal_state' in cell_states_to_model.")
                raise

            if cell_states_to_model["start_state"] == cell_states_to_model["goal_state"]:
            ...
            if "alt_states" in set(cell_states_to_model.keys()):
                if cell_states_to_model["alt_states"] is not None:
                    if not isinstance(cell_states_to_model["alt_states"], list):
                        logger.error("cell_states_to_model['alt_states'] must be a list (even if it is one element).")
                        raise
                    if len(cell_states_to_model["alt_states"]) != len(set(cell_states_to_model["alt_states"])):
                        logger.error("All states must be unique.")
                        raise
            else:
